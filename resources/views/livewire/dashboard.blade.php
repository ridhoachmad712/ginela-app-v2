    ...
    <div class="mx-auto flex max-w-5xl flex-col gap-5 px-5 py-6 pb-24 md:pb-6">
        <div class="flex flex-wrap items-end gap-3">
            <div>
                ...
                <p class="text-sm text-ink-soft">{{ now()->translatedFormat('l, d F Y') }} · ringkasan hari ini</p>
            </div>
            <a href="/kasir" class="ml-auto flex items-center gap-2 rounded-2xl bg-accent-600 px-5 py-3 text-sm font-bold text-white shadow-sm hover:bg-accent-700">
        ...
        <div class="grid grid-cols-2 gap-3 lg:grid-cols-4">
            @foreach ([['Penjualan hari ini', rp($t['penjualan']), 'from-accent-600 to-sky-500'], ['Laba hari ini', rp($t['laba']), 'from-green-600 to-emerald-500'], ['Transaksi', number_format($t['jml']), 'from-slate-700 to-slate-500'], ['Item terjual', number_format($t['item']), 'from-slate-700 to-slate-500']] as [$lab, $val, $grad])
                    ...
                    <ul class="flex flex-col gap-2">
                        @foreach ($this->topToday as $i => $p)
                            <li class="flex items-center gap-3 text-sm">
                                <span class="grid h-6 w-6 flex-none place-items-center rounded-lg bg-surface-3 text-xs font-bold text-ink-soft">{{ $i + 1 }}</span>
